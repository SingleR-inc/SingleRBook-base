...
or we could use other heatmap plotting functions such as `plotXXX()` from `r Biocpkg("dittoSeq")`.

```{r grun-beta-heat-all, fig.width=10, fig.height=10, fig.cap="Heatmaps of log-expression values in the Grun dataset for all marker genes upregulated in each label in the Muraro reference dataset. Assigned labels for each cell are shown at the top."}
collected <- list()
