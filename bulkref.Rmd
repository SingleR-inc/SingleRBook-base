...
Our plan is to annotate each cell with the ImmGen reference dataset [@ImmGenRef] from the `r Biocpkg("celldex")` package.
This and other references were originally built into `r Biocpkg("SingleR")`
but have since been migrated to a separate package for more general use throughout Bioconductor.
