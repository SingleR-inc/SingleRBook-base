...
We also need to compute log-expression values for use in marker detection inside `SingleR()`.
...
```{r, echo=FALSE}
# Sanity check.
rdx <- clusterRand(sceG$cluster, pred.grun$labels, mode="index")
stopifnot(rdx > 0.8)
```

We proceed to the most important part of the analysis.
Yes, that's right, the $t$-SNE plot (Figure \@ref(unref-pancreas-label-tsne)).

```{r unref-pancreas-label-tsne, fig.cap="$t$-SNE plot of the Grun dataset, where each point is a cell and is colored by the assigned cluster. Reference labels are also placed on the median coordinate across all cells assigned with that label."}
set.seed(101010100)
